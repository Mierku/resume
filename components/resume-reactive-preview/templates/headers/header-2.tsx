...
import styles from '../styles/headers/header-2.module.scss'
import { HeaderPhoto } from './HeaderPhoto'

export function renderHeader2({ data, block, marginBottom, onNavigate, helpers }: HeaderRenderProps) {
  const paddingTop = 0
          ...
          >
            {block.meta.map((item, metaIndex) => (
              <span
                ...
                className={styles.h2MetaItem}
                {...helpers.getPreviewActionProps(onNavigate, { sectionId: 'basics', fieldKey: item.fieldKey })}
              >
                {item.text}
                {metaIndex < block.meta.length - 1 ? <span className={styles.h2MetaSep}> | </span> : null}
              </span>
            ))}
          </p>
        ) : null}
