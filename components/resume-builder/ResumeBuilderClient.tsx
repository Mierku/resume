...
import {
  ...
  type ResumeCompletenessResult,
} from "./workbench/resume-completeness";
...
const RichTextEditor = dynamic(
  () =>
    import("./controls/RichTextEditor/RichTextEditor").then(
...
function estimateTemplatePageCountForData(data: ResumeData) {
  ...
  if (
    !Number.isFinite(contentMetrics.contentMaxHeightPx) ||
    contentMetrics.contentWidthPx <= 1
  ) {
    return 1;
  }
...
function ScrubbableNumberInput({
  ...
  const adjustBySteps = (steps: number) => {
    ...
    const nextValue = clampToRange(
      currentNumeric + steps * config.step,
      config.min,
      config.max,
    );
  ...
  const moveScrub = (event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (!steps) return;
    const nextValue = clampToRange(
      drag.startValue + steps * config.step,
      config.min,
      config.max,
    );
    const formatted = formatNumericValue(nextValue, config.step);
...
function preventDefaultIfCancelable(event: PreventableEvent) {
  const nativeCancelable =
    "nativeEvent" in event ? event.nativeEvent?.cancelable : undefined;
...
function resolvePdfPageSizeMm(
  ...
  const ratio =
    canvas.width > 0 ? canvas.height / canvas.width : fixedHeightMm / widthMm;
  const dynamicHeightMm = Math.max(
    PDF_MIN_FREE_FORM_HEIGHT_MM,
    widthMm * ratio,
  );
...
function buildPdfInWorker(payload: {
  ...
  return new Promise((resolve, reject) => {
    ...
    const worker = new Worker(
      `/resume-pdf.worker.js?v=${PDF_WORKER_SCRIPT_VERSION}`,
    );
    ...
    const handleMessage = (event: MessageEvent<unknown>) => {
      ...
      if (
        workerPayload.type === "BUILD_PDF_SUCCESS" &&
        workerPayload.blob instanceof Blob
      ) {
        cleanup();
        resolve({
          ...
          size:
            typeof workerPayload.size === "number"
              ? workerPayload.size
              : workerPayload.blob.size,
          timings: Array.isArray(workerPayload.timings)
            ? workerPayload.timings
            : [],
        });
        return;
      }
...
function SaveStatusTag() {
  ...
  if (save.status === "error") {
    return (
      <span className="resume-save-status is-error">
        保存失败：{save.error || "未知错误"}
      </span>
    );
  }
  ...
  if (save.status === "saved" && lastSavedTime) {
    return (
      <span className="resume-save-status">云端已保存于 {lastSavedTime}</span>
    );
  }
    ...
    <div className="space-y-4">
      <div className="resume-basics-grid grid grid-cols-2 gap-2">
        ...
        <EditorAnchor
          sectionId="basics"
          fieldKey="picture"
          className="resume-basics-photo-field p-3"
        >
          <div className="resume-basics-photo-head">
            <span className="resume-anchor-label text-xs text-muted-foreground">
              证件照
            </span>
        ...
        <EditorAnchor sectionId="basics" fieldKey="workYears">
          ...
          >
            {WORK_YEAR_OPTIONS.map((option) => (
              <Option
                key={option.value || "empty-work-years"}
                value={option.value}
              >
                {option.label}
              </Option>
        ...
        <EditorAnchor sectionId="basics" fieldKey="maritalStatus">
          ...
          >
            {MARITAL_STATUS_OPTIONS.map((option) => (
              <Option
                key={option.value || "empty-marital"}
                value={option.value}
              >
                {option.label}
              </Option>
        ...
        <EditorAnchor sectionId="basics" fieldKey="nativePlace">
          ...
          <Input
            ...
            onChange={(value) => updateField("nativePlace", value)}
            placeholder="请输入籍贯"
          />
        </EditorAnchor>
        ...
        <EditorAnchor sectionId="basics" fieldKey="politicalStatus">
          ...
          >
            {POLITICAL_STATUS_OPTIONS.map((option) => (
              <Option
                key={option.value || "empty-politics"}
                value={option.value}
              >
                {option.label}
              </Option>
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        ...
        <EditorAnchor sectionId="intention" fieldKey="intentionCity">
          ...
          <Input
            value={basics.intentionCity}
            onChange={(value) => updateField("intentionCity", value)}
            placeholder="例如：上海"
          />
        </EditorAnchor>
        ...
        <EditorAnchor sectionId="intention" fieldKey="intentionSalary">
          ...
          <Input
            value={basics.intentionSalary}
            onChange={(value) => updateField("intentionSalary", value)}
        ...
        <EditorAnchor sectionId="intention" fieldKey="intentionAvailability">
          ...
          <Select
            ...
            onChange={(value) =>
              updateField("intentionAvailability", toSingleSelectValue(value))
            }
            style={{ width: "100%" }}
          >
...
function SkillsSectionEditor() {
  ...
  const reorderItem = useResumeBuilderStore(
    (state) => state.reorderStandardSectionItem,
  );
  const updateResumeData = useResumeBuilderStore(
    (state) => state.updateResumeData,
  );
  const updateItem = useResumeBuilderStore(
    (state) => state.updateStandardSectionItem,
  );
  const removeItem = useResumeBuilderStore(
    (state) => state.removeStandardSectionItem,
  );
  const sortSensors = useEditorItemSortSensors();
  const sortableItemIds = useMemo(
    () =>
      section.items.map((item, index) =>
        resolveEditorItemId(item.id, `skills-${index}`),
      ),
    [section.items],
  );
  ...
  return (
    <div className="space-y-3">
      ...
      {section.items.length > 0 ? (
                    ...
                    {(dragHandle) => (
                      <div className="resume-soft-card resume-skill-inline-card">
                        <div className="resume-skill-inline-content">
                          <div className="resume-skill-inline-drag">
                            {dragHandle}
                          </div>
                          ...
                          >
                            <Input
                              value={item.name}
                              onChange={(value) =>
                                updateItem("skills", index, { name: value })
                              }
                              placeholder="技能名称"
                            />
                          </EditorAnchor>
                          ...
                          >
                            <Select
                              value={
                                item.proficiency || DEFAULT_SKILL_PROFICIENCY
                              }
                              onChange={(value) =>
        ...
        <div className="resume-skill-inline-empty text-xs text-muted-foreground">
          点击下方“新增条目”添加技能词条
        </div>
      )}
    </div>
...
function StandardSectionEditor({
  ...
  const section = useResumeBuilderStore(
    (state) => state.data.sections[sectionId],
  );
  const reorderItem = useResumeBuilderStore(
    (state) => state.reorderStandardSectionItem,
  );
        ...
        >
          <div className="space-y-3">
            {section.items.map((item, index) => {
              ...
              const itemId = resolveEditorItemId(
                item.id,
                `${sectionId}-${index}`,
              );
              const summary = resolveStandardSectionItemSummary(
                sectionId,
                record,
              );
              const collapsed =
                collapseEnabled && collapsedItemIds.includes(itemId);
                ...
                >
                  {(dragHandle) => (
                    ...
                    >
                      ...
                      >
                        <div className="resume-standard-item-head-main">
                          ...
                          {collapseEnabled ? (
                            <span
                              className={joinClassNames(
                                "resume-standard-item-head-toggle",
                          ...
                          {summary ? (
                            ...
                            >
                              <span className="resume-standard-item-summary-primary">
                                {summary.primary}
                              </span>
                              <span className="resume-standard-item-summary-sep">
                                /
                              </span>
                              <span className="resume-standard-item-summary-secondary">
                                {summary.secondary}
                              </span>
                            </div>
                          ) : null}
                      ...
                      <CollapseMotion
                        open={!collapsed}
                        className="resume-standard-item-collapse"
                      >
                        <div className="resume-standard-item-content">
                          {fields.map((field) => {
                            ...
                            if (field.key === "period") {
                              const { start, end } = splitPeriodValue(
                                String(currentValue || ""),
                              );
                                    ...
                                    onChange={(nextStart, nextEnd) => {
                                      updateItem(
                                        ...
                                        createNestedPatch(
                                          record,
                                          field.key,
                                          joinPeriodValue(nextStart, nextEnd),
                                        ),
                                      );
                                    }}
                                  />
...
function CustomSectionInlineEditor({ sectionId }: { sectionId: string }) {
  ...
  const reorderCustomItem = useResumeBuilderStore(
    (state) => state.reorderCustomSectionItem,
  );
  const updateCustomItem = useResumeBuilderStore(
    (state) => state.updateCustomSectionItem,
  );
  const removeCustomItem = useResumeBuilderStore(
    (state) => state.removeCustomSectionItem,
  );
  const sortSensors = useEditorItemSortSensors();

  if (!section) {
    return (
      <div className="text-xs text-muted-foreground">该自定义板块不存在</div>
    );
  }

  const sortableItemIds = section.items.map((item, index) =>
    resolveEditorItemId(item.id, `${section.id}-${index}`),
  );
  const handleDragEnd = (event: DragEndEvent) => {
    const indexes = resolveItemReorderIndexes(sortableItemIds, event);
    if (!indexes) return;
                  ...
                  {(dragHandle) => (
                    ...
                    >
                      <div className="resume-custom-inline-item-head">
                        <div className="resume-custom-inline-item-drag">
                          {dragHandle}
                        </div>
                        <Space>
                          <Button
                      ...
                      {section.type === "cover-letter" ? (
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <RichTextEditor
                              value={String(
                                (item as unknown as { recipient?: string })
                                  .recipient || "",
                              )}
                              onChange={(value) =>
                          ...
                          >
                            ...
                            <RichTextEditor
                              value={String(
                                (item as unknown as { content?: string })
                                  .content || "",
                              )}
                              onChange={(value) =>
                                updateCustomItem(section.id, index, {
...
function AddRowButton({
  ...
  return (
    <button
      type="button"
      className="resume-add-row-button"
      onClick={onClick}
      disabled={disabled}
    >
...
function resolvePreviewFitTarget(previewContent: HTMLDivElement | null) {
  ...
  const compareStage = previewContent.querySelector<HTMLElement>(
    ".resume-ai-compare-stage",
  );
  if (compareStage) {
    return compareStage;
  }
  ...
  return (
    previewContent.querySelector<HTMLElement>(".resume-preview-root") ||
    previewContent
  );
}
...
function resolvePreviewFitScale(
  ...
  const availableWidth = Math.max(
    viewport.clientWidth - PREVIEW_FIT_HORIZONTAL_PADDING * 2,
    0,
  );
  const availableHeight = Math.max(
    viewport.clientHeight - PREVIEW_FIT_HEIGHT_PADDING * 2,
    0,
  );
...
function resolvePreviewScrollMax(
  viewport: HTMLDivElement | null,
...
function resolveCenteredScrollTop(
  ...
  const viewportCenter =
    viewport.scrollTop + viewport.clientHeight / 2 - verticalPadding;
...
export function ResumeBuilderClient({
  ...
  const [previewInteractionActive, setPreviewInteractionActive] =
    useState(false);
  ...
  const [aiPreviewState, setAiPreviewState] = useState<AIPreviewState | null>(
    null,
  );
  ...
  const [heightDebugSnapshot, setHeightDebugSnapshot] =
    useState<HeightDebugSnapshot | null>(null);
  ...
  const activeAIDraftId =
    activeTool === "ai" ? aiPreviewState?.draftId : undefined;
  ...
  const previewScrollSpaceHeight =
    previewScaledHeight + PREVIEW_SCROLL_VERTICAL_PADDING * 2;
  ...
  const handlePreviewDraftInCanvas = useCallback(
    (payload: {
      draftId: string;
      sourceResumeId: string;
      title: string;
    ...
    async (payload: {
      ...
      try {
        const response = await fetch(
          `/api/resumes/${encodeURIComponent(payload.draftId)}`,
        );
        ...
        const normalized = normalizeResumeContent(draftResume.content, {
          dataSource:
            dataSources.find((item) => item.id === draftResume.dataSourceId) ||
            null,
  ...
  const runPreviewDraftAction = useCallback(
    async (action: "new_version" | "overwrite" | "discard") => {
      ...
      try {
        if (action === "discard") {
          const response = await fetch(
            `/api/ai/drafts/${encodeURIComponent(activeAIDraftId)}/discard`,
            {
              method: "POST",
            },
          );
          ...
          const response = await fetch(
            `/api/ai/drafts/${encodeURIComponent(activeAIDraftId)}/confirm`,
            {
              method: "POST",
          ...
          Message.success(
            action === "overwrite" ? "已覆盖原简历" : "已保存为新版本",
          );
        }
  ...
  const setPreviewScaleCentered = useCallback((nextScaleRaw: number) => {
    ...
    const nextScrollSpaceHeight =
      previewContentHeightRef.current * nextScale +
      PREVIEW_SCROLL_VERTICAL_PADDING * 2;
    const maxScrollTop = resolvePreviewScrollMax(
      viewport,
      nextScrollSpaceHeight,
    );
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      const step =
        event.deltaY < 0 ? PREVIEW_ZOOM_STEP_WHEEL : -PREVIEW_ZOOM_STEP_WHEEL;
  ...
  useEffect(() => {
    ...
    const runtimePayload =
      authRedirectRuntimeDraft &&
      now - authRedirectRuntimeDraft.cachedAt <=
        AUTH_REDIRECT_RUNTIME_DRAFT_MAX_AGE_MS
    ...
    const isExpired =
      ...
      now - payload.savedAt > AUTH_REDIRECT_DRAFT_MAX_AGE_MS;
    if (isExpired) {
      window.sessionStorage.removeItem(AUTH_REDIRECT_DRAFT_CACHE_KEY);
      authRedirectRuntimeDraft = null;
      return;
    ...
    if (
      (!sameResume && !samePath && !samePathname && !guestToAuthedEditorFlow) ||
      !payload.data ||
    ...
    if (process.env.NODE_ENV !== "production" && source === "storage") {
      authRedirectRuntimeDraft = {
        payload: {
          ...
          selectedDataSourceId:
            typeof payload.selectedDataSourceId === "string"
              ? payload.selectedDataSourceId
              : "",
          data: restoredData,
        },
        cachedAt: now,
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      EDITOR_PANEL_WIDTH_STORAGE_KEY,
      String(nextWidth),
    );
  }, [editorPanelWidth]);
  ...
  const handleEditorPanelResizeStart = useCallback(
  ...
  const handleEditorPanelResizeEnd = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      const drag = editorPanelResizeRef.current;
      if (!drag || drag.pointerId !== event.pointerId) return;
      ...
      setEditorPanelWidth((previous) =>
        previous === editorPanelWidthLiveRef.current
          ? previous
          : editorPanelWidthLiveRef.current,
      );
      if (typeof document !== "undefined") {
        document.body.classList.remove("resume-editor-panel-resizing");
      }
  ...
  const exportResumePagesAsImage = async (format: ResumeImageExportFormat) => {
    try {
      ...
      const previewRoot = previewContentRef.current?.querySelector<HTMLElement>(
        ".resume-preview-root",
      );
      if (!previewRoot) {
      ...
      const pages = Array.from(
        previewRoot.querySelectorAll<HTMLElement>("[data-template]"),
      );
      if (pages.length === 0) {
      ...
      const normalizedTitle =
        (resumeTitleRef.current || initialResume.title || "resume")
          .trim()
          .replace(/[^\w\u4e00-\u9fa5-]+/g, "-") || "resume";
      ...
      try {
        ...
        const capturePages = Array.from(
          captureRoot.querySelectorAll<HTMLElement>("[data-template]"),
        );
        ...
        const exportWidth = Math.max(
          ...renderedCanvases.map((canvas) => canvas.width),
        );
        const exportHeight = renderedCanvases.reduce(
        ...
        renderedCanvases.forEach((canvas) => {
          const drawLeft = Math.max(
            0,
            Math.floor((exportWidth - canvas.width) / 2),
          );
        ...
        const blob = await new Promise<Blob | null>((resolve) =>
          stitchedCanvas.toBlob(resolve, mimeType, quality),
        );
        if (!blob) {
          throw new Error("图片生成失败");
        }
  ...
  const exportResumePagesAsPdf = async () => {
    ...
    try {
      ...
      const snapdom = await profiler.measure("load snapdom", () =>
        loadSnapdom(),
      );
      const previewRoot = previewContentRef.current?.querySelector<HTMLElement>(
        ".resume-preview-root",
      ...
      const { captureHost, captureRoot } = await profiler.measure(
        "clone preview root",
        () => createOffscreenExportCaptureRoot(previewRoot),
      );
      try {
        ...
        for (const [index, page] of capturePages.entries()) {
          const canvas = await profiler.measure(
            `page ${index + 1} toCanvas`,
        ...
        for (const [index, canvas] of renderedCanvases.entries()) {
          const imageBlob = await profiler.measure(
            `page ${index + 1} toBlob(${PDF_EXPORT_IMAGE_FORMAT})`,
            () =>
              canvasToBlob(
                canvas,
                PDF_EXPORT_IMAGE_MIME,
          ...
          const imageBuffer = await profiler.measure(
            `page ${index + 1} blob.arrayBuffer`,
        ...
        Message.success(
          `已下载 PDF（${renderedCanvases.length} 页，${formatDurationMs(totalDurationMs)}，详情见控制台）`,
        );
      } finally {
        captureHost.remove();
      }
  ...
  const editorPanelContent = (
    <IntegratedSectionsEditor
      ...
      renderBasicsEditor={() => <BasicsEditor />}
      renderIntentionEditor={() => <IntentionEditor />}
