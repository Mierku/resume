    ...
    const loadDataSource = async () => {
      try {
        ...
        if (res.ok) {
          ...
          toast.error('数据源不存在')
          router.push('/builder/data-source')
        }
      } catch {
        toast.error('加载失败')
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (isNew) {
          const data = await res.json()
          router.push(`/builder/data-source/${data.dataSource.id}`)
        }
      } else if (res.status === 401) {
      ...
      <>
        <div className="mx-auto max-w-[980px] px-6 py-14">
          <div className="rounded-[12px] border border-border bg-background/70 p-8 text-center">
            ...
            <div className="mt-4">
              <Link href={`/login?next=${encodeURIComponent(pathname || '/builder/data-source')}`}>
        ...
        <AuthRequiredModal
          ...
          redirectPath={pathname || '/builder/data-source'}
        />
      </>
    )
  }
